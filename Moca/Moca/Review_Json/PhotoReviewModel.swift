//
//  PhotoReviewModel.swift
//  Moca
//
//  Created by Ria Song on 2021/03/15.
//
...
// insertModel에선 필요없다 (?)
protocol PhotoReviewProtocol: class{
    func photoReviewitemDownloaded(items: NSArray) // <- 여기에 담은 아이템을 아래 delegate에서 사용하고, tableView에서 궁극적으로 사용.
}

class PhotoReviewModel: NSObject{
    var delegate: PhotoReviewProtocol!
    let urlPath = "http://" + Share.macIP + "/moca/jsp/review_photo_list.jsp" // 리뷰 전체 불러오기
    ...
    func parseJSON(_ data: Data){
        ...
        DispatchQueue.main.async(execute: {() -> Void in
            self.delegate.photoReviewitemDownloaded(items: locations)
        })
    }
} // END
